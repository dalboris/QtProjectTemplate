...
# Generate subdirs project file for tests
for relDir in projects:
    # Get project
    project = projects[relDir]

    # For all subdirs in libs/ (including libs itself)
    if project.template == "subdirs" and project.relDir.startswith('libs'):

        # Get directory of this test subdir
        testProjectDir = os.path.join(testsDir, relDir)

        # Create directory for this test subdir
        mkdir_p(testProjectDir)

        # Get file for this test project
        testProjectFilepath = os.path.join(testProjectDir, project.filename)

        # Generate new text for this project file.
        testProjectText  = "# This file is automatically generated. Any edit will be lost.\n"
        testProjectText += "# It is kept in source control for readability on GitHub.\n\n"
        testProjectText += "TEMPLATE = subdirs\n"
        testProjectText += "SUBDIRS ="
        for subdir in sorted(project.subdirs):
            testProjectText += " \\\n    " + subdir
        testProjectText += "\n"

        # Get existing text if the project file already exists.
        if os.path.isfile(testProjectFilepath):
            f = open(testProjectFilepath, 'r')
            testProjectOldText = f.read()
            f.close()
        else:
            testProjectOldText = ""

        # If the new text and the existing text differ, then write new text to file
        if testProjectText != testProjectOldText:
            f = open(testProjectFilepath, 'w')
            f.write(testProjectText)
            f.close()
